...
type UserOption = MultipleComboboxOption<string> & {
  name: string;
  username: string;
};
...
export function ShareCopyDialog({ data }: DialogProps<'resume.shareCopy'>) {
  ...
  const [selectedOptionsDetail, setSelectedOptionsDetail] = useState<
    UserOption[]
  >([]);
  ...
  const { mutateAsync: shareCopy, isPending } = useMutation(
    orpc.resume.shareCopy.mutationOptions()
  );

  const searchResultOptions = useMemo(() => users.map(userToOption), [users]);

  const onValueChange = useCallback(
    (values: string[], optionsSelected: MultipleComboboxOption<string>[]) => {
      setSelectedIds(values);
      setSelectedOptionsDetail((prev) =>
        values
          .map(
            (id) =>
              (optionsSelected.find((o) => o.value === id) as
                | UserOption
                | undefined) ?? prev.find((o) => o.value === id)
          )
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      const { count } = await shareCopy({
        resumeId: data.resumeId,
        userIds: selectedIds
      });
      toast.success(t`A copy was shared with ${count} user(s).`, {
        id: toastId
      });
      closeDialog();
    } catch (error) {
  ...
  return (
    <DialogContent className="flex max-h-[85vh] flex-col">
      <DialogHeader>
        ...
        <DialogDescription>
          <Trans>
            Search and select users to send a copy. They will receive the resume
            in their dashboard.
          </Trans>
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="flex flex-1 flex-col gap-2">
        ...
        <MultipleCombobox
          ...
          emptyMessage={
            isSearchingUsers
              ? t`Searching...`
              : t`No users found. Type to search.`
          }
