...
export function ImportResumeDialog(_: DialogProps<'resume.import'>) {
  ...
  const { mutateAsync: importResume } = useMutation(
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (values.type === 'pdf') {
        if (!isAIEnabled)
          throw new Error(
            t`This feature requires AI Integration to be enabled. Please enable it in the settings.`
          );
      ...
      if (error instanceof Error) {
        ...
        toast.error(t`An unknown error occurred while importing your resume.`, {
          id: toastId,
          description: null
        });
      }
  ...
  return (
    <DialogContent {...blockEvents}>
      <DialogHeader>
        ...
        <DialogDescription>
          <Trans>
            Continue where you left off by importing an existing resume you
            created using Reactive Resume or any another resume builder.
            Supported formats include PDF, Microsoft Word, as well as JSON files
