...
export function DialogManager() {
  ...
  const dialogContent = match(activeDialog)
    ...
    .with({ type: 'domain.update' }, ({ data }) => (
      <UpdateDomainDialog data={data} />
    ))
    .with({ type: 'customer.create' }, () => <CreateCustomerDialog />)
    .with({ type: 'customer.update' }, ({ data }) => (
      <UpdateCustomerDialog data={data} />
    ))
    .with({ type: 'position.create' }, () => <CreatePositionDialog />)
    .with({ type: 'position.update' }, ({ data }) => (
      <UpdatePositionDialog data={data} />
    ))
    .with({ type: 'skill.create' }, () => <CreateSkillDictionaryDialog />)
    .with({ type: 'skill.update' }, ({ data }) => (
      <UpdateSkillDictionaryDialog data={data} />
    ))
    .with({ type: 'resume.create' }, () => <CreateResumeDialog />)
    .with({ type: 'project.create' }, () => <CreateDashboardProjectDialog />)
    .with({ type: 'project.update' }, ({ data }) => (
      <UpdateDashboardProjectDialog data={data} />
    ))
    .with({ type: 'resume.update' }, ({ data }) => (
      <UpdateResumeDialog data={data} />
    ))
    .with({ type: 'resume.duplicate' }, ({ data }) => (
      <DuplicateResumeDialog data={data} />
    ))
    .with({ type: 'resume.shareCopy' }, ({ data }) => (
      <ShareCopyDialog data={data} />
    ))
    ...
    .with({ type: 'resume.sections.summary.create' }, ({ data }) => (
      <CreateSummaryItemDialog data={data} />
    ))
    .with({ type: 'resume.sections.summary.update' }, ({ data }) => (
      <UpdateSummaryItemDialog data={data} />
    ))
    .with({ type: 'resume.sections.cover-letter.create' }, ({ data }) => (
      <CreateCoverLetterDialog data={data} />
    ))
    .with({ type: 'resume.sections.cover-letter.update' }, ({ data }) => (
