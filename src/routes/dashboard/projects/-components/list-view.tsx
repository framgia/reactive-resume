...
function ProjectListItem({ project }: { project: Project }) {
  ...
  return (
    <>
      <div
        className={cn(
          'flex h-12 items-center rounded-md px-1',
          isDeleted && 'bg-muted/50 opacity-75'
        )}>
      ...
      <p className="flex h-12 min-w-32 shrink-0 items-center px-1 text-xs opacity-60">
        {isDeleted ? (
          <Trans>Deleted on {deletedAt}</Trans>
        ) : (
...
function ProjectRestoreButton({ project }: { project: Project }) {
  const { mutate: restoreProject, isPending } = useMutation(
