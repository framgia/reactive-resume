...
export const Route = createFileRoute('/builder/$resumeId')({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [{ title: `${loaderData.name} - Sun Asterisk Global CV` }]
      : undefined
  })
});
...
function BuilderLayout({ initialLayout, ...props }: BuilderLayoutProps) {
  ...
  const { maxSidebarSize, collapsedSidebarSize } = useBuilderSidebar(
    (state) => ({
...
const getBuilderLayoutServerFn = createServerFn({ method: 'GET' }).handler(
  async () => {
