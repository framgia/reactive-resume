...
import { BriefcaseIcon, CaretRightIcon, CaretUpDownIcon, ChartBarIcon, FolderIcon } from "@phosphor-icons/react";
...
export function OverallSectionBuilder() {
	...
	const collapsed = useSectionStore((state) => state.sections.overall?.collapsed ?? false);
	...
	const invalidateResume = () =>
		queryClient.invalidateQueries({
			queryKey: orpc.resume.getById.queryOptions({ input: { id: resumeId } }).queryKey,
		});

	const handleProjectChange = async (next: string | null) => {
		const nextProjectId = next === null ? null : next;
		await updateResume({
			id: resumeId,
			...
			<AccordionItem value="overall" className="group/accordion space-y-4">
				<div className="flex items-center">
					...
					<div className="flex flex-1 items-center gap-x-4">
						...
						<h2 className="line-clamp-1 font-bold text-2xl tracking-tight">{getSectionTitle("overall")}</h2>
					</div>
				</div>
							...
							<>
								<div className="space-y-2">
									...
									<OverallMultiSelect
										...
										displayLabels={resumeSkills?.map((s) => s.name) ?? []}
...
function OverallMultiSelect({
	...
	const display = displayLabels.length > 0 ? displayLabels.join(", ") : placeholder;
...
function OverallOptionSelect({
	...
	const display: string = displayLabel ?? options.find((o) => o.id === value)?.name ?? placeholder;
